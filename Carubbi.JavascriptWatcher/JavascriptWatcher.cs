...
namespace Carubbi.JavascriptWatcher
{
    ...
    {
        private readonly WebBrowser _context;
        ...
        private void AttachHandlerImpl()
        {
            ...
            var js = @"window.alertProxy = window.alert;
            ...
            var el = _context.Document?.CreateElement("script");
            var dom = (IHTMLScriptElement)el?.DomElement;
            if (dom == null) return;
            dom.text = js;
            ...
            var jqueryEl = _context.Document.CreateElement("script");
            dom.src = "//ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js";
            if (_context.Document.Body != null)
            {
                _context.Document.Body.AppendChild(jqueryEl ?? throw new InvalidOperationException());
            }
        }

        protected virtual void OnAlertIntercepted(AlertInterceptedEventArgs e)
        {
            AlertIntercepted?.Invoke(this, e);
        }

        protected virtual void OnWindowOpenIntercepted(WindowOpenInterceptedEventArgs e)
        {
            WindowOpenIntercepted?.Invoke(this, e);
        }

        protected virtual void OnConfirmIntercepted(ConfirmInterceptedEventArgs e)
        {
            ConfirmIntercepted?.Invoke(this, e);
        }
    }
}
